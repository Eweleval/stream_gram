...
class __UserProfileState extends State<_UserProfile> {
  ...
  late Future<StreamgramUser> userDataFuture = getUser();

  Future<StreamgramUser> getUser() async {
    ...
    return StreamgramUser.fromMap(streamUser.data!);
  }
  ...
  @override
  Widget build(BuildContext context) {
    return FutureBuilder<StreamgramUser>(
    ...
    return Row(
      children: [
        Padding(
          ...
          child: Avatar.medium(streamgramUser: widget.userData),
        ),
        Padding(
          padding: const EdgeInsets.all(8.0),
          child: Column(
