...
/// State related to Stream-gram app.
///
...
class AppState extends ChangeNotifier {
  ...
  StreamgramUser? _streamgramUser;
  var isUploadingProfilePicture = false;

  /// The extraData from [user], mapped to an [StreamgramUser] object.
  StreamgramUser? get streamgramUser => _streamgramUser;
  ...
  Future<bool> connect(DemoAppUser demoUser) async {
    ...
    if (currentUser.data != null) {
      _streamgramUser = StreamgramUser.fromMap(currentUser.data!);
  ...
  ///
  /// This will call [notifyListeners] and update the local [_streamgramUser] state.
  Future<void> updateProfilePhoto(String filePath) async {
    ...
    _streamgramUser = _streamgramUser?.copyWith(
    ...
    if (_streamgramUser != null) {
      await client.currentUser!.update(_streamgramUser!.toMap());
    }
